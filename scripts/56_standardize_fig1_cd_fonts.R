...
deg <- read.csv(file.path(panel_dir, "Fig1A_source_data.csv"), check.names = FALSE)
...
# B: unchanged WGCNA module-trait association content, redrawn at final slot size.
assoc <- read.csv(file.path(root, "sepsis-s100a8-multiomics", "data", "03_WGCNA_analysis", "output", "module_group_association.csv"))
...
# C: same black-module eigengene values and statistics, typography corrected for final size.
cdat <- read.csv(file.path(panel_dir, "Fig1C_source_data.csv"))
...
# D: same corrected 200-gene overlap and colors, typography corrected for final size.
overlap <- scan(file.path(panel_dir, "corrected_overlap_200_genes.txt"), what = character(), quiet = TRUE)
black <- scan(file.path(root, "sepsis-s100a8-multiomics", "data", "03_WGCNA_analysis", "output", "DIFF_MODULE_black_GENES.txt"), what = character(), quiet = TRUE)
sig <- filter(deg, Regulation != "Not significant")$Gene
only_black <- length(setdiff(black, sig)); only_deg <- length(setdiff(sig, black))
stopifnot(length(overlap) == 200, only_black == 537, only_deg == 1308)
...
pD <- ggplot() +
  ...
  annotate("text", x = 0.27, y = 0.50, label = only_black, family = font, fontface = "bold", size = 8 / .pt) +
  annotate("text", x = 0.50, y = 0.50, label = length(overlap), family = font, fontface = "bold", size = 8 / .pt) +
  annotate("text", x = 0.73, y = 0.50, label = only_deg, family = font, fontface = "bold", size = 8 / .pt) +
...
go_all <- read.csv(file.path(panel_dir, "Fig1E_GO_all_results.csv"), check.names = FALSE)
...
kegg_all <- read.csv(file.path(panel_dir, "Fig1F_KEGG_all_results.csv"), check.names = FALSE)
...
save_single(pD, "Fig1D_overlap_200_font_standardized", 3.42, 2.19)

candidate_pdf <- file.path(panel_dir, "Fig1_CD_font_standardized_candidate.pdf")
candidate_svg <- file.path(panel_dir, "Fig1_CD_font_standardized_candidate.svg")
qa_ascii <- "C:/Users/ZCL/CodexQA_sepsis_cd"
dir.create(qa_ascii, recursive = TRUE, showWarnings = FALSE)
candidate_tif <- file.path(qa_ascii, "Fig1_CD_font_standardized_candidate.tif")
