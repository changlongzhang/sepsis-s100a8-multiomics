...
panel_dir <- file.path(root, "work", "final_revision_20260908", "figure_sources", "Fig1_S1_allgenes")
formal_dir <- "C:/Users/ZCL/final_revision_outputs"
...
# Freeze the DEG calls to the exact corrected result table used by the final
# all-gene WGCNA rerun; expression is still read independently for modelling.
deg <- read.csv(
  file.path(root, "work", "final_revision_20260908", "wgcna_allgenes_rerun", "corrected_DEG_results_used.csv"),
  check.names = FALSE
) %>% dplyr::select(Gene, baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, Regulation)
sig <- deg %>% filter(Regulation != "Not significant")
...
black <- unique(readLines(file.path(
  root, "work", "final_revision_20260908", "wgcna_allgenes_rerun", "primary_sepsis_module_black_genes.txt"
), warn = FALSE))
black <- black[nzchar(black)]
overlap <- unique(readLines(file.path(
  root, "work", "final_revision_20260908", "wgcna_allgenes_rerun", "primary_sepsis_module_corrected_DEG_overlap.txt"
), warn = FALSE))
overlap <- overlap[nzchar(overlap)]
stopifnot(length(overlap) == 265, setequal(overlap, intersect(black, sig$Gene)), "S100A8" %in% overlap)

writeLines(sig$Gene, file.path(panel_dir, "corrected_DEGs_1508_genes.txt"))
writeLines(overlap, file.path(panel_dir, "corrected_overlap_265_genes.txt"))
...
save_gg_panel(venn, "Fig1D_overlap_265", 6, 6)
...
cat("S1_ALL4\t", paste(Reduce(intersect, list(lasso_genes, rf_genes, boost_genes, xgb_genes)), collapse = ","), "\n", sep = "")
  ...
  load(file.path(panel_dir, "S1_corrected_analysis.RData"))
  path_df <- read.csv(file.path(panel_dir, "S1_lasso_path_corrected.csv"), check.names = FALSE)
}
cat("STAGE\tS1_PLOTS\n")
cv_df <- data.frame(
...
pE <- ggplot(wf) +
  ...
  annotate("text", x = baseline, y = max(wf$Y) + 0.35, label = sprintf("Baseline = %.3f", baseline),
           hjust = ifelse(baseline > final_margin, 1, 0), size = 8 / ggplot2::.pt, colour = mid, family = font_family) +
  annotate("text", x = final_margin, y = max(wf$Y) + 0.35, label = sprintf("Prediction = %.3f", final_margin),
           hjust = ifelse(final_margin > baseline, 1, 0), size = 8 / ggplot2::.pt, colour = ink, family = font_family) +
...
ggsave(file.path(formal_dir, "S1_fig_preserved_style.pdf"), s1, width = 7.5, height = 8.75,
       device = cairo_pdf, family = font_family, bg = "white")
