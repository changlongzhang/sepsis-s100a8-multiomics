...
out_dir <- file.path(root, "work", "final_revision_20260908", "wgcna_meanprobe_sensitivity")
...
cat("WGCNA MEAN-PROBE AGGREGATION SENSITIVITY ANALYSIS\n")
cat("Started:", format(Sys.time(), tz = "Asia/Shanghai"), "\n")
cat("R:", R.version.string, "\n")
cat("WGCNA:", as.character(packageVersion("WGCNA")), "\n")
cat("Seed: 20260908\n")

input_matrix <- file.path(root, "review_revision", "02_class_imbalance", "GSE65682_gene_log2_mean_probes.rds")
...
cat("Reading existing log2 RMA mean-probe gene matrix without modifying source file...\n")
expr_log2 <- as.matrix(readRDS(input_matrix))
storage.mode(expr_log2) <- "double"
if (any(!is.finite(expr_log2))) stop("The mean-probe matrix contains non-finite values.")
gc()
...
input_summary <- data.frame(
  ...
  value = c("log2 RMA", "mean probe per gene", nrow(expr_log2), ncol(expr_log2),
...
# Comparability-preserving gene QC: retain every gene with finite, nonzero
# variance. This avoids a new variance ranking in the 760:42 cohort and keeps
# the same gene scope as the archived WGCNA without any target-gene exception.
gene_mad <- rowMads(expr_log2, na.rm = TRUE)
gene_var <- rowVars(expr_log2, na.rm = TRUE)
mad_cut <- NA_real_
keep_low_info <- is.finite(gene_mad) & is.finite(gene_var) & gene_var > 0
filter_audit <- data.frame(
  ...
  retained = keep_low_info, reason = ifelse(!is.finite(gene_mad) | !is.finite(gene_var), "nonfinite_dispersion",
                                    ifelse(gene_var <= 0, "zero_variance", "retained"))
)
...
cat("MAD threshold: none; finite nonzero-variance QC only\n")
...
parameters <- data.frame(
  ...
  value = c("log2 RMA", "mean probe per gene", "all genes with finite, nonzero variance; no label-based filtering",
            ...
            20260908, 12000, length(unique(net$blocks)))
)
...
saveRDS(expr_log2, file.path(out_dir, "GSE65682_log2_RMA_genelevel_meanprobe_sensitivity.rds"), compress = "xz")
