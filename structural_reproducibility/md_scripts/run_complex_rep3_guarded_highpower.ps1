...
if (-not (Test-Path -LiteralPath $tpr)) { throw "Missing production TPR: $tpr" }
...
while ($true) {
    ...
    if (Test-Path -LiteralPath $cpt) { $args += @('-cpi',$cpt) }
    ...
    if ($exit -ne 0) {
        ...
        throw "Production segment failed with exit code $exit"
    }
