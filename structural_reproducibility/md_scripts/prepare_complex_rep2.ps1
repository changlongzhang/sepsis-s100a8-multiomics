...
Run-Gmx 'NVT grompp' {
    & $gmx grompp -f (Join-Path $mdp 'nvt_complex_rep2.mdp') `
...
Run-Gmx 'NPT grompp' {
    & $gmx grompp -f (Join-Path $mdp 'npt_complex_rep2.mdp') `
...
Run-Gmx 'Production grompp' {
    & $gmx grompp -f (Join-Path $mdp 'md_100ns_complex_rep2.mdp') `
...
"PREPARED $(Get-Date -Format o) gen_seed=207985 nvt_ld_seed=207985 npt_ld_seed=207986 production_ld_seed=207987" |
