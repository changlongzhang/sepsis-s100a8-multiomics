...
$outRoot = Join-Path $root 'comparative_analysis_v2'
...
try {
    ...
    foreach ($job in $jobs) {
        ...
        Run-Step "$tag backbone fit after PBC clustering" (Join-Path $out 'FIT.done') {
            @('4','0') | & $gmx trjconv -s $subsetTpr -f $clustered -n $subsetIndex -o $fit -fit rot+trans
        } @($fit)
        ...
        $com = Join-Path $out 'chainB_chainD_com_distance_0_100ns.xvg'
        Run-Step "$tag chain B-D COM distance" (Join-Path $out 'CHAIN_COM.done') {
            & $gmx distance -s $subsetTpr -f $fit -n $chainIndex -select 'com of group "chainB" plus com of group "chainD"' -oall $com -tu ns
        } @($com)
        ...
        Run-Step "$tag inter-chain hydrogen bonds" (Join-Path $out 'INTERCHAIN_HB.done') {
            @('0','1') | & $gmx hbond -s $subsetTpr -f $fit -n $chainIndex -num $interHb -b 20000 -e 100000 -dt 100
        } @($interHb)
        ...
        if ($job.system -eq 'complex') {
            ...
            $ligRmsd = Join-Path $out 'ligand_rmsd_after_proteinfit_0_100ns.xvg'
            Run-Step "$tag ligand RMSD after protein fit" (Join-Path $out 'LIGAND_RMSD.done') {
                @('15') | & $gmx rms -s $subsetTpr -f $fit -n $subsetIndex -o $ligRmsd -tu ns -fit none
            } @($ligRmsd)
            Require-Rows "$tag ligand RMSD" $ligRmsd 10001

            $pocket = Join-Path $out 'ligand_original_pocket_com_distance_0_100ns.xvg'
            Run-Step "$tag ligand-original-pocket COM distance" (Join-Path $out 'POCKET_DISTANCE.done') {
                & $gmx distance -s $subsetTpr -f $fit -n $pocketIndex -select 'com of group "ligand" plus com of group "pocket"' -oall $pocket -tu ns
            } @($pocket)
            Require-Rows "$tag pocket distance" $pocket 10001

            $mindist = Join-Path $out 'ligand_protein_mindist_0_100ns.xvg'
            $contacts = Join-Path $out 'ligand_protein_contacts_0_100ns.xvg'
            Run-Step "$tag ligand-protein contacts" (Join-Path $out 'LIGAND_CONTACTS.done') {
                @('1','15') | & $gmx mindist -s $subsetTpr -f $fit -n $subsetIndex -od $mindist -on $contacts -d 0.45 -tu ns
            } @($mindist,$contacts)
            ...
            Run-Step "$tag corrected ligand-protein hydrogen bonds" (Join-Path $out 'LIGAND_HB.done') {
                @('1','15') | & $gmx hbond -s $subsetTpr -f $fit -n $subsetIndex -num $ligHb -b 20000 -e 100000 -dt 100
            } @($ligHb)
            Require-Rows "$tag ligand-protein H-bonds" $ligHb 801

            $occ = Join-Path $out 'protein_residue_ligand_contact_occupancy_0p4nm_20_100ns.xvg'
            $selection = 'res_com of (group "Protein" and same residue as (group "Protein" and within 0.4 of group "TPL"))'
            Run-Step "$tag corrected residue contact occupancy" (Join-Path $out 'CONTACT_OCCUPANCY.done') {
                & $gmx select -s $subsetTpr -f $fit -n $subsetIndex -select $selection -of $occ -b 20000 -e 100000 -dt 100 -xvg none
            } @($occ)
            Require-Rows "$tag residue contact occupancy" $occ 186
        }

        "PASS $(Get-Date -Format o); pbc=cluster; center=Protein; frames=10001" | Set-Content -LiteralPath (Join-Path $out 'TRAJECTORY_METRICS_COMPLETE.txt') -Encoding UTF8
        Log "ALL METRICS COMPLETE $tag"
    }

    "completed=$(Get-Date -Format o); trajectories=6; pbc=cluster; canonical_inputs_preserved=true" | Set-Content -LiteralPath (Join-Path $outRoot 'PBC_CORRECTED_METRICS_COMPLETE.txt') -Encoding UTF8
    "ALL_PBC_CORRECTED_METRICS_COMPLETE`t$(Get-Date -Format o)" | Set-Content -LiteralPath $stage -Encoding UTF8
    Log 'ALL_PBC_CORRECTED_METRICS_COMPLETE'
} catch {
